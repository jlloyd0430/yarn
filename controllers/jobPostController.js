...
// In your jobPostController.js file
...
exports.allJobPostGetRoutes = async (req, res) => {
  try {
    ...
    return res.status(200).render("allJobPost", {
      title: "allJobPost",
      jobPosts, // Use a plural name for an array of job posts
    });
  } catch (error) {
    return res.status(404).redirect("/404");
  }
...
exports.postRequestDeleteGetRoutes = async (req, res) => {
  try {
    let jobPost = await JobPost.findById(req.params.jobPostId).populate({
      path: "userId",
      select: "username profileImage",
    });
...
exports.postRequestEditPutRoutes = async (req, res) => {
  try {
    const user = await User.findOne({ username: req.params.username });
    let jobPost = await JobPost.findById(req.params.jobPostId);
    if (!user || !jobPost) return res.status(404).redirect("/404");
